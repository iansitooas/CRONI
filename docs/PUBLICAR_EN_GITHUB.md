...
3. Confirma que la versión de `Cargo.toml` sea la que quieres publicar, por ejemplo `0.5.0`.
...
1. Cambia `version` en `Cargo.toml`, por ejemplo a `0.5.1`.
2. Ejecuta `cargo check` y `cargo test`.
3. Sube los cambios a GitHub.
4. Crea y sube `v0.5.1`.
