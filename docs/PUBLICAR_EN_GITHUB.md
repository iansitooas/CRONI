...
3. Confirma que la versión de `Cargo.toml` sea la que quieres publicar, por ejemplo `0.4.7`.
...
1. Cambia `version` en `Cargo.toml`, por ejemplo a `0.4.8`.
2. Ejecuta `cargo check` y `cargo test`.
3. Sube los cambios a GitHub.
4. Crea y sube `v0.4.8`.
