...
El evento `ContainsFullScreenElementChanged` de WebView2 controla la pantalla completa HTML5. Al entrar, CRONI oculta la barra nativa, expande el mismo WebView y usa pantalla completa sin bordes; al salir restaura la ventana y la barra sin navegar ni recrear la pestaña. Cada cambio de tamaño notifica al compositor mediante `NotifyParentWindowPositionChanged`. En Windows, una regla inyectada reduce únicamente el elemento de video 2 px por lado mientras está en pantalla completa. La diferencia visual es imperceptible y evita que el plano de video coincida exactamente con toda la superficie interna de WebView2, condición que puede detener la presentación del compositor y dejar la imagen negra.
