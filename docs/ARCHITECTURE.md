...
Desde 0.5.7 el bloqueo sigue activo por defecto y las excepciones manuales por sitio se conservan. Red, navegación y filtros visuales respetan esas excepciones. La limpieza publicitaria se inicia con `DOMContentLoaded` según la política nativa actual, no incondicionalmente desde un script de arranque. Al desactivar, se desconecta el observador, se retira el CSS y se recarga la pestaña activa para recuperar elementos ya retirados. La lógica de pausa multimedia permanece independiente.

Para guardar documentos y páginas, el menú usa `ICoreWebView2_25::ShowSaveAsUI`. Se mantiene `SuppressDefaultDialog=false`: el [selector nativo de Guardar como](https://learn.microsoft.com/en-us/microsoft-edge/webview2/reference/win32/icorewebview2saveasuishowingeventargs?view=webview2-1.0.4022.49) solicita destino también en esa ruta. Las descargas ordinarias siguen pasando por el diálogo propio sin filtrar extensión ni MIME. Los runtimes antiguos mantienen su comportamiento nativo; el nuevo comando requiere una versión que admita esa API.
