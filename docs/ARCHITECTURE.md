...
En Windows se reemplaza `requestFullscreen()` por un modo de ventana completa simulado dentro del documento. El elemento solicitado ocupa el viewport dejando un margen negro imperceptible de 2 px por lado: evita que el video sea candidato a la ruta defectuosa de superposición de superficie completa de WebView2. Un canal IPC limitado exclusivamente a un booleano indica a CRONI cuándo debe ocultar o restaurar la barra nativa. `F` y `Esc` siguen funcionando, pero WebView2 nunca activa su capa HTML5 de pantalla completa, que en algunos equipos deja de presentar imagen y permanece negra incluso al salir. La ventana principal tampoco cambia de modo. Cada cambio de tamaño notifica al compositor mediante `NotifyParentWindowPositionChanged`, y al restaurar la interfaz se fuerza una presentación nueva. Además se desactivan la composición GPU y DirectComposition para evitar la falla conocida de presentación en WebView2 sobre Windows 11 build 26200. La decodificación de video por hardware continúa disponible, aunque la composición final por software puede consumir algo más de CPU.
