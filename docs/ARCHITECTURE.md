...
El evento `ContainsFullScreenElementChanged` de WebView2 controla la pantalla completa HTML5. Al entrar, CRONI oculta la barra nativa, expande el mismo WebView y usa pantalla completa sin bordes; al salir restaura la ventana y la barra sin navegar ni recrear la pestaña. Cada cambio de tamaño notifica al compositor mediante `NotifyParentWindowPositionChanged`. En Windows se conserva un margen físico negro de 3 px: evita que el plano de video coincida exactamente con toda la superficie del host, una combinación capaz de dejar negra la presentación de WebView2.
