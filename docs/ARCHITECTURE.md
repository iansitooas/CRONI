...
El evento `ContainsFullScreenElementChanged` de WebView2 controla la pantalla completa HTML5. Al entrar, CRONI oculta la barra nativa y expande el mismo WebView a toda el área de la ventana; al salir restaura la barra sin navegar ni recrear la pestaña. La ventana principal no cambia al modo de pantalla completa de Windows, porque esa transición puede invalidar la superficie de WebView2 y mantenerla negra incluso después de salir. Cada cambio de tamaño notifica al compositor mediante `NotifyParentWindowPositionChanged`, y al restaurar la interfaz se fuerza además una presentación nueva. En Windows se usa `--disable-gpu-compositing` para evitar otra falla de DirectComposition que puede dejar negra o congelada la superficie de video. La decodificación de video por hardware continúa disponible, aunque la composición final por software puede consumir algo más de CPU.
