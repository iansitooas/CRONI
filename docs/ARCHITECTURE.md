...
El evento `ContainsFullScreenElementChanged` de WebView2 controla la pantalla completa HTML5. Al entrar, CRONI oculta la barra nativa, expande el mismo WebView y usa pantalla completa sin bordes; al salir restaura la ventana y la barra sin navegar ni recrear la pestaña. Cada cambio de tamaño notifica al compositor mediante `NotifyParentWindowPositionChanged`. En Windows se usa `--disable-gpu-compositing` para evitar una falla de presentación de DirectComposition que puede dejar negra o congelada la superficie de video. La decodificación de video por hardware continúa disponible, aunque la composición final por software puede consumir algo más de CPU.
